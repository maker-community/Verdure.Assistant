...
public class NAudioRecorder : IAudioRecorder, IDisposable
{
    ...
    public async Task StartRecordingAsync(int sampleRate = 16000, int channels = 1)
    {
        ...
        try
        {
            ...
            {
                WaveFormat = new WaveFormat(sampleRate, 16, channels), // 16位深�?
            ...
            _logger?.LogInformation("音频录制已启�? {SampleRate}Hz, {Channels}声道", sampleRate, channels);
    ...
    public async Task StopRecordingAsync()
    {
        ...
        try
        {
            ...
            _logger?.LogInformation("音频录制已停");

            await Task.CompletedTask;
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "停止音频录制时出");
            Console.WriteLine($"停止音频录制时出 {ex.Message}");
        }
    }
    ...
    private void OnRecordingStopped(object? sender, StoppedEventArgs e)
    {
        try
        {
            ...
            if (e.Exception != null)
            {
                _logger?.LogError(e.Exception, "录制过程中发生错");
            }
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "处理录制停止事件时出");
        }
    }
        ...
        try
        {
            ...
            lock (_lock)
            {
                if (_waveIn != null)
                {
                    ...
                    {
                        _logger?.LogWarning(ex, "释放WaveIn时出");
                    }
                }
                ...
                {
                    _logger?.LogWarning(ex, "清理录制数据时出");
                }
            }
            
            _isRecording = false;
            _logger?.LogDebug("NAudioRecorder资源已释");
        }
        catch (Exception ex)
        {
            _logger?.LogError(ex, "释放NAudioRecorder资源时出");
        }
    }
}
