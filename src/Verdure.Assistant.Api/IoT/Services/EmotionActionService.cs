...
/// </summary>
public class EmotionActionService : IEmotionActionService
{
    ...
    public async Task<bool> PlayEmotionWithActionAsync(PlayRequest request)
    {
        ...
        // 验证表情类型
        if (!EmotionTypes.IsValid(mappedEmotion))
        {
            _logger.LogError($"无效的表情类型: {emotionType} -> {mappedEmotion}");
            return false;
        }
        ...
        _logger.LogInformation($"开始播放情感 {emotionType} -> {mappedEmotion}，包含动作: {request.IncludeAction}，包含表情: {request.IncludeEmotion}");
        ...
        try
        {
            ...
            {
                tasks.Add(PlayEmotionOnlyAsync(mappedEmotion, request.Loops, request.Fps, cancellationToken));
            }
            
            if (request.IncludeAction)
            {
                tasks.Add(PlayActionOnlyAsync(mappedEmotion, cancellationToken));
            }
        ...
        catch (OperationCanceledException)
        {
            ...
            _logger.LogInformation($"情感 {mappedEmotion} 播放被取消");
            return false;
        }
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, $"情感 {mappedEmotion} 播放发生错误");
            return false;
        }
    }
    ...
    {
        var mappedEmotion = EmotionMappingService.MapEmotion(emotionType);
        _logger.LogInformation($"开始播放表情动画: {emotionType} -> {mappedEmotion}");
        ...
        try
        {
            await _displayService.PlayEmotionAsync(mappedEmotion, loops, fps, cancellationToken);
            return true;
        }
        catch (OperationCanceledException)
        {
            _logger.LogInformation($"表情动画 {mappedEmotion} 播放被取消");
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"表情动画 {mappedEmotion} 播放失败");
            return false;
        }
    }
    ...
    {
        var mappedEmotion = EmotionMappingService.MapEmotion(emotionType);
        _logger.LogInformation($"开始播放动作: {emotionType} -> {mappedEmotion}");
        ...
        try
        {
            await _robotActionService.PerformActionAsync(mappedEmotion, cancellationToken);
            return true;
        }
        catch (OperationCanceledException)
        {
            _logger.LogInformation($"动作 {mappedEmotion} 播放被取消");
            return false;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, $"动作 {mappedEmotion} 播放失败");
            return false;
        }
    }
    ...
    public IEnumerable<EmotionConfig> GetAvailableEmotionConfigs()
    {
        ...
        return availableEmotions.Where(e => _emotionConfigs.ContainsKey(e))
                                .Select(e => _emotionConfigs[e]).ToList();
    }
