using System;
using System.Threading.Tasks;
using Microsoft.Extensions.Logging;

namespace Verdure.Assistant.Core.Services
{
    /// <summary>
    /// 模拟相机服务 - 用于测试和不支持的平台
    /// </summary>
    public class MockCameraService : ICameraService
    {
        private readonly ILogger<MockCameraService> _logger;

        public MockCameraService(ILogger<MockCameraService> logger)
        {
            _logger = logger;
        }

        public async Task<byte[]> CapturePhotoAsync(CameraSettings? settings = null)
        {
            _logger.LogInformation("Mock camera capturing photo with settings: {Settings}", 
                settings?.Width + "x" + settings?.Height ?? "default");

            // 模拟拍照延迟
            await Task.Delay(500);

            // 返回一个最小的有效JPEG文件（1x1像素）
            var mockJpegBytes = new byte[]
            {
                
            };

            _logger.LogInformation("Mock camera photo captured. Size: {Size} bytes", mockJpegBytes.Length);
            return mockJpegBytes;
        }

        public async Task<bool> IsAvailableAsync()
        {
            await Task.Delay(100); // 模拟检查延迟
            return true; // 模拟相机始终可用
        }

        public async Task<string[]> GetSupportedResolutionsAsync()
        {
            return await Task.FromResult(new[]
            {
                "320x240",
                "640x480", 
                "800x600",
                "1024x768",
                "1280x720",
                "1920x1080"
            });
        }

        public async Task<CameraInfo> GetCameraInfoAsync()
        {
            var info = new CameraInfo
            {
                Name = "Mock Camera",
                Device = "mock",
                Platform = "Mock",
                IsAvailable = true,
                SupportedFormats = new[] { "JPEG" },
                SupportedResolutions = await GetSupportedResolutionsAsync()
            };

            return info;
        }
    }
}
